...
class ChromBPNetBatchGenerator(keras.utils.Sequence):
    ...
    """
    def __init__(self, peak_regions, nonpeak_regions, genome_fasta, batch_size, inputlen, outputlen, max_jitter, negative_sampling_ratio, cts_bw_file, add_revcomp, return_coords, shuffle_at_epoch_start ,save_data = False, mode = "train"):
        """
        seqs: B x L' x 4
        cts: B x M'
        ...
        if save_data:
            #save directory as save_data/mode/peak_seqs.npy
            full_path = os.path.join(save_data, mode)
            if not os.path.exists(full_path):
                os.makedirs(full_path)
            np.save(full_path+'/peak_seqs', peak_seqs)
            np.save(full_path+'/peak_cts', peak_cts)
            np.save(full_path+'/peak_coords', peak_coords)
            np.save(full_path+'/nonpeak_seqs', nonpeak_seqs)
            np.save(full_path+'/nonpeak_cts', nonpeak_cts)
            np.save(full_path+'/nonpeak_coords', nonpeak_coords)
